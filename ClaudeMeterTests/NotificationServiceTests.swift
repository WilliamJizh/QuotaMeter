...
final class NotificationServiceTests: XCTestCase {
    ...
    func test_userReceivesResetNotificationWhenUsageResets() async {
        ...
        state["account-1/five_hour"] = NotificationState.Window(lastPercentage: 50)
...
/// A single account with a single primary window, so each assertion counts
/// notifications for exactly one limit.
@MainActor
private func makeUsageData(percentage: Double) -> UsageData {
    UsageFixtures.usageData(
        accounts: [UsageFixtures.account(windows: [UsageFixtures.window(percentage: percentage)])]
    )
}
