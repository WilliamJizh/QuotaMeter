            ...
            if isLoading {
                ...
                HStack(spacing: segmentSpacing) {
                    ForEach(0..<segmentCount, id: \.self) { index in
                        ...
                        let isActive = remaining >= threshold - (100.0 / Double(segmentCount))
...
#Preview {
    HStack(spacing: 20) {
        SegmentedBarIcon(remaining: 20, status: .safe, isLoading: false, isStale: false)
        SegmentedBarIcon(remaining: 45, status: .safe, isLoading: false, isStale: false)
        SegmentedBarIcon(remaining: 65, status: .warning, isLoading: false, isStale: false)
        SegmentedBarIcon(remaining: 92, status: .critical, isLoading: false, isStale: false)
        SegmentedBarIcon(remaining: 45, status: .safe, isLoading: true, isStale: false)
        SegmentedBarIcon(remaining: 45, status: .safe, isLoading: false, isStale: true)
    }
    .padding()
}
