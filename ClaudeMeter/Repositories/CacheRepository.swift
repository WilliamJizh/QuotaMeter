...
actor CacheRepository: CacheRepositoryProtocol {
    ...
    init(fileManager: FileManager = .default) {
        ...
        let cacheDir = appSupport.appendingPathComponent("com.quotameter", isDirectory: true)
        try? fileManager.createDirectory(at: cacheDir, withIntermediateDirectories: true)

        self.diskCacheURL = cacheDir.appendingPathComponent("usage_cache.json")

        // Public JSON export at ~/.quotameter/usage.json for external tools
        let homeDir = fileManager.homeDirectoryForCurrentUser
        let publicDir = homeDir.appendingPathComponent(".quotameter", isDirectory: true)
